const ANNOUNCEMENT = "IMPORTANT: Our hosting service will likely change URLs to end in <span style='font-family:monospace'>.replit.app</span> by January 1st. For more information <a style='color:white' href='https://blog.replit.com/hosting-changes' target='_blank'>click here</a>. In case of emergency, the website will also stay up-and-running at <a style='color:white' href='https://ulbcwebsite.github.io' target='_blank'>ulbcwebsite.github.io</a>."
...
switch (page) {
  ...
  case 'leaderboard':
    onValue(ref(db, "users"), (snapshot) => {
      ...
      var wordsAmtArray = testThingy.map(a=>a.bio.split(" ").length)
      ...
      sortedEmails.forEach((email, idx) => {
        document.querySelector("ol").innerHTML+=`<li><a href="/profile/?${combined[idx].uid}">`+email+"</a> - "+sortedNumbers[idx]+" words, "+combined[idx].views+" views</li>"
      })
    })
    break
  case 'profile':
...
onAuthStateChanged(auth, (user) => {
  if (user) {
    ...
    if (page == "biobuilder") {
      ...
      get(ref(db, "users/"+user.uid+"/bio")).then((snapshot) => {
        document.querySelector("#main textarea").value=snapshot.val()
        document.getElementById("wordCount").innerText="Words: "+snapshot.val().split(" ").length||0
        document.getElementById("charCount").innerText="Characters: " + snapshot.val().length||0
